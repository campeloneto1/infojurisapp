...
export default function CadastrarAudiencia({route}) {
  ...
  async function getAudiencia(){
    ...
    fetch(env.default.url+'audiencias/'+route.params?.id,{
      method: 'GET',
      headers: {
    ...
    .then((json) => {   
      //console.log(json)                
      if(json){
        setProcesso(json.processo_id);
        setTipo(json.tipo_id);
        setData(json.data.substr(8,2)+json.data.substr(5,2)+json.data.substr(0,4));
        ...
        if(json){
          ...
          if(json == 1){
            Toast.show({
              ...
              text2: 'Audiência editada com sucesso.',
              autoHide: true,
              visibilityTime: 2000
            });
            clear();
